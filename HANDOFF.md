...
- 下一步：用户下载 fork Release 的 macOS 版替换 `/Applications/LLM Wiki.app`（替换前退出应用；wiki 数据在项目目录 `.llm-wiki/`，替换无损；未签名 dmg 首次打开需右键「打开」）；以后修改直接推 fork `main`，发版打 `v*` 标签自动构建 Release；T2 侧栏文件夹分组（六大类）评估待启动。
